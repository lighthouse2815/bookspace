...
Join, unpublish và delete dùng cùng serialized, non-deferred SQLite challenge-mutation boundary, lấy write lock trước khi đọc điều kiện và giữ đến commit. Nếu join commit trước thì unpublish/delete nhận conflict vì đã có participant; nếu admin mutation commit trước thì join nhận conflict hoặc not-found vì challenge không còn hợp lệ. Database không thể commit trạng thái challenge draft/đã xóa đồng thời có participant. Leave cũng xóa participation, đồng bộ trạng thái còn lại và map DTO trong một transaction; controller không đọc lại sau commit. Publish `true` và cập nhật nội dung không thuộc boundary hẹp này.
